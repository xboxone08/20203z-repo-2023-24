...
void Drive::turn_to_angle(float angle, float turn_max_voltage, float turn_settle_error, float turn_settle_time, float turn_timeout, float turn_kp, float turn_ki, float turn_kd, float turn_starti){
  ...
  //PID turnPID(reduce_negative_180_to_180(angle - get_absolute_heading()), 0.08, 1.0, 0.05, 0.0, 0.5, 500.0, turn_timeout);
  while(turnPID.is_settled() == false){
    float error = reduce_negative_180_to_180(angle - get_absolute_heading());
    //printf("error = %+7.3f, ", error);
    //printf("gyro = %+7.3f, ", Gyro.rotation()*360.0/gyro_scale);
    //printf("abs = %+7.3f\n", get_absolute_heading());
    float output = turnPID.compute(error);
    output = clamp(output, -turn_max_voltage, turn_max_voltage);
    //output = clamp(output, -8.0, 8.0);
